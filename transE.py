...
def dataloader(file1, file2, file3):
    '''
    使用id表示元组中的实体和关系，均由列表存储
    :param file1: 三元组文件
    :param file2: 实体id文件
    :param file3: 关系id文件
    :return: 实体id，关系id和元组(均由id表示)列表
    '''
    print("load file...")
...
class TransE:
    def __init__(self, entity, relation, triple_list, embedding_dim=5, lr=0.01, margin=1.0, norm=1):
    ...
    def data_initialise(self):
        '''
        对实体向量和关系向量进行初始化，以字典形式存储，键为实体或者关系的id，值为其向量
        :return: 实体向量和关系向量的初始化
        '''
        entityVectorList = {}
        relationVectorList = {}
    ...
    def normalization(self, vector):
        '''
        对向量进行标准化
        :param vector: 需标准化的向量
        :return: 标准化后的向量
        '''
        return vector / np.linalg.norm(vector)

    def training_run(self, epochs=500, nbatches=40, out_file_title = ''):
        '''

        :param epochs: 训练的次数
        :param nbatches: 每个batch的元组个数
        :param out_file_title: 选定训练的数据集
        :return: None
        '''
        ...
        with codecs.open(out_file_title +"TransE_entity_" + str(self.dimension) + "dim_epochs" + str(epochs) + '.json', "w",encoding="utf-8") as f1:
            for e in self.entities.keys():
                f1.write(e + "\t")
                f1.write(str(list(self.entities[e])))
                f1.write("\n")

        with codecs.open(out_file_title +"TransE_relation_" + str(self.dimension) + "dim_epochs" + str(epochs) + '.json', "w",encoding="utf-8") as f2:
    ...
    def update_triple_embedding(self, Tbatch):
        ...
        for correct_sample, corrupted_sample in Tbatch:
            #copy_entity和copy_relation为更新的实体或者关系向量，self.entities为初始的实体或者关系向量
            ...
            # calculate the distance of the triples，利用初始的实体或者关系向量对每个元组的距离进行计算？
            if self.norm == 1:
...
if __name__ == '__main__':
    ...
    file1 = r"../元组和实体构建/三元组.json"
    file2 = r"../元组和实体构建/实体id.json"
    file3 = r"../元组和实体构建/关系id.json"
    entity_set, relation_set, triple_list = dataloader(file1, file2, file3)

    transE = TransE(entity_set, relation_set, triple_list, embedding_dim=50, lr=0.05, margin=1.0, norm=2)
    transE.data_initialise()
    transE.training_run(out_file_title="图嵌入表示_")
